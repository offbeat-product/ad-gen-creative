...
const N8N_WEBHOOK_URL =
  'https://offbeat-inc.app.n8n.cloud/webhook/adgen-spot-banner-image';
...
const BannerImageTool = () => {
  ...
  const [creativeStyle, setCreativeStyle] = useState('photographic');
  ...
  const [variationsPerSize, setVariationsPerSize] = useState(3);
  const [burnText, setBurnText] = useState(true);
  const [outputPng, setOutputPng] = useState(true);
  const [outputPsd, setOutputPsd] = useState(true);
  ...
  const handleRestoreJob = useCallback(
    (jid: string, _projectId: string, inputData: Record<string, unknown>) => {
      ...
      if (inputData.sizes && Array.isArray(inputData.sizes)) {
        setSelectedSizes(inputData.sizes as BannerSize[]);
      }
      ...
      renderSettings={({ context }) => {
        ...
        const handleGenerate = async () => {
          if (!state.projectId || !user) {
            toast.error('案件情報が不足しています');
            return;
          }
          if (!mainCopy.trim()) {
            toast.error('メインコピーは必須です');
            return;
          }
