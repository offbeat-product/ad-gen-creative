...
const ProductCard = ({
  ...
  product: { id: string; name: string; label: string };
  selected: boolean;
  onClick: () => void;
}) => {
...
const SpotStepProduct = ({ state, updateState, goToStep }: Props) => {
    ...
    <div className="space-y-6">
      ...
      {client && (
        <div className="flex items-center gap-2 text-sm">
          ...
          <button onClick={() => goToStep(0)} className="text-secondary text-xs hover:underline">
            変更
          </button>
        </div>
      )}
...
export default SpotStepProduct;
